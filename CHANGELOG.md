...
- Pin-capability model: new builtins `pin()`, `peripheral()`, `pool()`, `pin_request()` and `pin_solve()` declare what a multi-function component's pins can do and solve the instance × pin assignment deterministically at elaboration (exclusivity at both tiers, costs, `previous=` stability). Per-candidate realization `data` (e.g. STM32 AF indices) is carried verbatim into the result; the solved `pin_assignment` and residual `swap_classes` are stored as module properties (JSON) and flow into the netlist. The solver is a deterministic branch-and-bound returning the minimum-total-cost assignment. `pin_request(if_connected=True)` serves a request only when the caller connected the matching `io(optional=True)` slot, `pin_map()` turns the assignment into a `Component(pins=...)`-ready dict, `at(value, pins, soft=)` lets a caller constrain the physical pin on the connection itself (`Mcu(IO0 = at(led_net, "PA8"))`), `pin_solve` returns `free_pins` (candidate pins no request claimed) so components wire unused mux pins without a separate list, and `pin_request(bind=)` supports dict-of-roles demands: the design names pooled-capability roles (`Mcu(gpio = {"LED": at(led_net, "PA8")})`) instead of pre-declared numbered slots.
- `interface(implies = [...])`: an interface type can imply others; capability matching is nominal and closed over the implication DAG (a `Usart` provider satisfies a `Uart` request, never the reverse). `interface(attrs = {...})` declares the capability-attribute vocabulary (name -> physical type): peripheral attrs are validated against it — unknown keys and wrong dimensions are elaboration errors (stdlib `Uart` declares `baud_max: Frequency`).
