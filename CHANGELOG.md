...
- `pcb datasheet <QUERY>` deterministically resolves a component to its datasheet and prints the
  ...
  web:components`), a reference designator (workspace only), or an MPN (resolved through workspace
  packages, the local registry index, the KiCad symbol index, then the component search API).
