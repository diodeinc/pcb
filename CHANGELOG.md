...
- Added `pcb rectifier check` and `pcb rectifier fix`, backed by a bundled `pcb-rectifier` sidecar binary, to check and patch KiCad footprint 3D model `(rotate ...)`/`(offset ...)` transforms by matching tessellated STEP geometry against the footprint's pads and holes.
