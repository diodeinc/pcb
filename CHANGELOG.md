...
- Pin-capability model: new builtins `pin()`, `peripheral()`, `pool()`, `pin_request()` and `pin_solve()` declare what a multi-function component's pins can do and solve the instance × pin assignment deterministically at elaboration (exclusivity at both tiers, costs, `previous=` stability). Per-candidate realization `data` (e.g. STM32 AF indices) is carried verbatim into the result; the solved `pin_assignment` and residual `swap_classes` are stored as module properties (JSON) and flow into the netlist.
