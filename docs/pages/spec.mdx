...
Additional net types (`Analog`, `Pwm`, `Gpio`) are available from
...
Net types follow automatic conversion rules across `io()` boundaries:
`NotConnected` promotes to any type, specialized types demote to `Net`, and
`Net` does not automatically promote to specialized types. For explicit casting,
call the target constructor with an existing net as the first positional
