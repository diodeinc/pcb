...
application roles (LED, buzzer, button) are net names on the design side and
must never become capability interfaces. And **component modules expose
generic slots** (`IO0 = io(Net, optional = True)` requesting `Gpio`), because
a reusable component cannot know what the design will hang on them.
