...
**Module Methods**:
- `module.nets()` - Returns dict mapping net names to lists of connected port tuples
- `module.components()` - Returns dict mapping component paths to component objects
- `module.graph()` - Returns CircuitGraph object for path finding and topology validation
...
updated = voltage.with_value(5)           # Change to 5V, keep unit and tolerance
...
# Comparisons
check(voltage > Voltage("2V"), "Voltage too low")     # Compare with string
check(resistance == "10kOhm", "Wrong resistance")     # Equality with string
sorted_voltages = sorted([v1, v2, v3])                # Sort physical values
```
...
- `with_value(value)` - Update the numeric value while preserving unit and tolerance, returns new PhysicalValue
...
**Comparisons**:
- Physical values support equality (`==`, `!=`) and ordering (`<`, `>`, `<=`, `>=`) operations
- Units must match for comparisons, except dimensionless values can compare with any unit
- Tolerance is ignored in equality comparisons - only value and unit matter
- Can compare with strings (e.g., `voltage == "5V"`) if they represent compatible physical values
...
### TestBench(name, module, test_cases, checks)

Creates a test bench for validating module behavior across multiple test cases with parametric inputs.

```python
# Load a module to test
MyCircuit = Module("./my_circuit.zen")

# Define check functions that receive (module, inputs)
def verify_power_connections(module, inputs):
    """Ensure all power pins are connected to VCC"""
    nets = module.nets()
    vcc_connections = nets.get("VCC", [])
    check(len(vcc_connections) >= 2, "Need at least 2 VCC connections")

def verify_ground_plane(module, inputs):
    """Check for proper ground connections"""
    nets = module.nets()
    check("GND" in nets, "Missing GND net")
    check(len(nets["GND"]) > 3, "GND net needs more than 3 connections")

# Create test bench with multiple test cases
TestBench(
    name = "PowerTest",
    module = MyCircuit,
    test_cases = {
        "case_3v3": {"voltage": "3.3V", "current": "1A"},
        "case_5v": {"voltage": "5V", "current": "2A"},
    },
...
- `module`: ModuleLoader to test (created with `Module()`)
- `test_cases`: Dict mapping case names to input parameter dicts
- `checks`: Optional list of check functions to execute for each test case

**Check Function Signature:**

Check functions receive two arguments: the evaluated module and the test case inputs:

```python
def check_function(module: Module, inputs: dict):
    # Access circuit data by calling Module methods
    nets = module.nets()              # Returns dict of net connections
    components = module.components()  # Returns dict of component objects
    graph = module.graph()            # Returns CircuitGraph object

    # Access test case parameters
...
- `module.nets()` - Returns dict mapping net names to lists of connected port tuples
  - Example: `{"VCC": [("U1", "VDD"), ("C1", "P1")]}`
- `module.components()` - Returns dict mapping component paths to component objects
  - Example: `{"U1.IC": <Component>, "C1.C": <Component>}`
- `module.graph()` - Returns CircuitGraph for path finding and topology validation
...
- Evaluates the module once for each test case with the provided inputs
- Executes all check functions for each successful module evaluation
- Reports failures as diagnostics with precise source location
- Prints a summary message showing passed/failed checks across all test cases
- Returns a TestBench value containing results

**Example with Multiple Test Cases:**

```python
MyModule = Module("./voltage_regulator.zen")

def check_output_voltage(module, inputs):
    """Verify output voltage configuration"""
    components = module.components()
    regulator = components.get("U1.REG")

    expected_voltage = inputs["output_voltage"]
    check(regulator is not None, "Regulator component not found")

    print(f"✓ Output voltage configured for {expected_voltage}")

def check_bypass_capacitors(module, inputs):
    """Verify proper decoupling"""
    graph = module.graph()

    # Find paths from VIN to GND through capacitors
    vin_paths = graph.paths(start=("U1", "VIN"), end="GND_GND")
    check(len(vin_paths) > 0, "No bypass capacitor found on VIN")

    print(f"✓ Found {len(vin_paths)} bypass capacitor paths")

def check_component_existence(module, inputs):
    """Verify required components are present"""
    if "U1" in module:
        regulator = module["U1"]
        print(f"✓ Found regulator: {regulator}")
    else:
        error("Regulator not found")

TestBench(
    name = "VoltageRegulatorTest",
    module = MyModule,
    test_cases = {
        "3v3_config": {
            "output_voltage": "3.3V",
...
You can provide custom names for check functions using tuples:
...
TestBench(
    ...
    test_cases = {"default": {}},
    checks = [
        ("Power Check", check_power),
        ("Ground Check", check_ground),
    ]
)
```
...
```python
def analyze_circuit(module, inputs):
    # Get the circuit graph from the module
    graph = module.graph()
...
```python
def validate_power_supply(module, inputs):
    graph = module.graph()
...
```python
def validate_filter_topology(module, inputs):
    graph = module.graph()
...
```python
def validate_vin_decoupling(module, inputs):
...
**Feedback Networks**
```python
def validate_feedback_divider(module, inputs):
...
**Bootstrap Circuits**
```python
def validate_bootstrap_cap(module, inputs):
