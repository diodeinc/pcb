...
- **`pin_request(name, Iface, uses=, instance=, prefer=, lock=, where=, direction=, if_connected=)`**
  ...
  works — unit strings coerce in physical-value comparisons). `instance=`,
...
- **`at(value, pins, soft=)`** — constrain which physical pin(s) a
  ...
  `Mcu(IO0 = at(led_net, "PA8"))`. The wrapper is transparent — `io()`
  unwraps it at binding time, the inner net flows as if passed directly, and
  the constraint reaches the module's `pin_solve` keyed by the input name.
