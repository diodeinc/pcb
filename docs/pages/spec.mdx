...
| `datasheet` | no | Datasheet URL or path (default: `part.datasheet`, then symbol `Datasheet` property; local explicit paths resolved relative to the `.zen` file, symbol-local paths resolved relative to the `.kicad_sym` file) |
