...
- **`pin_request(name, Iface, uses=, instance=, prefer=, lock=, where=, direction=)`**
  ...
  `prefer=`, `lock=` are caller-side overrides.
- **`pin_solve(peripherals, requests, config=, previous=)`** — deterministic
  joint instance × pin matching. Exclusivity is structural at both tiers: a
  pin carries at most one function, and an instance serves at most one request
  (two `Uart` requests can never share USART1, even on disjoint pin sets).
  The solver prefers the poorest sufficient instance, honors costs and
  `prefer=`, and takes `previous=` (a prior assignment) as a stability seed so
  adding a request does not reshuffle an already-routed design. Infeasibility
  is a build error with per-candidate rejection reasons.
