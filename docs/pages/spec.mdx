...
`http_mirror` is optional and supports template placeholders:
- `{repo}`
- `{repo_name}`
- `{version}`
- `{major}`
...
A small set of commonly used stdlib symbols are implicitly available in user `.zen` files without an explicit `load()` statement. These **prelude symbols** are:

- `Power` (from `@stdlib/interfaces.zen`)
- `Ground` (from `@stdlib/interfaces.zen`)
- `NotConnected` (from `@stdlib/interfaces.zen`)
- `Layout` (from `@stdlib/properties.zen`)
- `Board` (from `@stdlib/board_config.zen`)

```python
# Prelude symbols are available without loading them explicitly
...
Explicit `load()` statements for prelude symbols still work and are not an error:
...
Local definitions shadow prelude symbols using normal Starlark scoping rules:

```python
Power = "not the stdlib symbol"  # shadows the prelude Power
```

The prelude does not apply to stdlib modules themselves (only to user `.zen` files).
...
Component(
    ...
    manufacturer = "TI",           # Optional: manufacturer
    mpn = "LM358",                 # Optional: manufacturer part number
    part = builtin.Part(           # Optional: typed part metadata
        mpn = "LM358DR",
        manufacturer = "TI",
        qualifications = ["PPAP"],
    ),
    ...
    properties = {                 # Optional: additional properties
        "voltage": "5V",
        "alternatives": [          # Optional: list[Part] or legacy compatible formats
            builtin.Part(mpn = "LM358PWR", manufacturer = "TI"),
        ],
    }
)
```
...
- `part`: Typed part object from `builtin.Part(...)`
- `type`: Component type
- `properties`: Additional properties dict

Part sourcing rules:

- If `part=` is provided, it is canonical. `component.mpn` and `component.manufacturer` are taken from `part`.
- If `part=` is omitted, `mpn`/`manufacturer` resolve from: explicit kwargs, then legacy `properties`, then symbol metadata (`Manufacturer_Part_Number`, `Manufacturer_Name`).

Modifier-time mutability guarantees:

- `component.part = builtin.Part(...)` updates canonical scalar fields (`component.mpn`, `component.manufacturer`).
- mutating `component.mpn` / `component.manufacturer` keeps `component.part` aligned when present.
- `component.alternatives` is mutable in modifiers (assignment and list operations like `.append(...)`).
...
- Also accept KiCad's `<lib>:<fp>` form and resolve it using the matching `[[workspace.kicad_library]]` entry (`symbols` -> `footprints`) for the symbol version's major.
- If no matching `[[workspace.kicad_library]]` major-version entry exists, inference fails with an error.
...
Note: `Power` and `Ground` are net types from `@stdlib/interfaces.zen`, not interfaces. They are available via the prelude (no explicit `load()` needed):
```python
load("@stdlib/units.zen", "Voltage")
vcc = Power("VCC_3V3", voltage=Voltage("3.3V"))
gnd = Ground("GND")
...
A `PhysicalValue` represents an electrical quantity with a nominal value, explicit min/max bounds, and a physical unit. The same type covers point values and ranges.
...
Create a typed part object for primary and alternative sourcing metadata.

**Signature:** `builtin.Part(*, mpn, manufacturer, qualifications=[])`

**Parameters:**
- `mpn`: Non-empty part number string
- `manufacturer`: Non-empty manufacturer string
- `qualifications`: Optional `list[str]` metadata, defaults to `[]`

**Returns:** `Part` with fields `.mpn`, `.manufacturer`, `.qualifications`.
...
Netlist compatibility requirements:

- Existing scalar component attributes `mpn` and `manufacturer` remain unchanged.
- Structured `part` is additive and serialized as JSON.
- `properties["alternatives"]` can be `list[Part]` and serializes as a JSON-object array (not `Part(...)` strings).
