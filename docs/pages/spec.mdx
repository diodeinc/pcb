...
def verify_power_connections(module):
    ...
    vcc_ports = [p for p, net in module.ports.items() if "VCC" in net]
...
Check functions receive a single `ModuleView` argument containing circuit data:

```python
def check_function(module):
    # Access circuit data through the ModuleView
    nets = module.nets              # Map of net names to connected ports
    ports = module.ports            # Map of port names to their nets  
    components = module.components  # Map of component names to attributes
...
- `module.nets`: Maps each net name to a list of connected port paths (e.g., `{"VCC": ["U1.VDD", "C1.P1"]}`)
- `module.ports`: Maps each port path to its connected net (e.g., `{"U1.VDD": "VCC"}`)
- `module.components`: Maps component paths to their attributes including pins, values, and properties
...
def check_resistor_values(module):
    ...
    for name, attrs in module.components.items():
...
def check_differential_pairs(module):
    """Verify differential pairs are properly routed"""
    diff_pairs = [("USB_DP", "USB_DN"), ("TX_P", "TX_N")]
    for pos, neg in diff_pairs:
        check(pos in module.nets and neg in module.nets, f"Differential pair {pos}/{neg} not found")
        # Check they connect to same component
        pos_comps = {p.split('.')[0] for p in module.nets[pos]}
        neg_comps = {p.split('.')[0] for p in module.nets[neg]}
...
TestBench(
    ...
    checks = [
        ...
        lambda module: "GND" in module.nets,  # Ensure ground exists
    ]
)
```
