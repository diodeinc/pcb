...
- `footprint`: PCB footprint. Optional only when a file-backed `symbol` provides an inferable `Footprint` property and `<symbol_dir>/<stem>.kicad_mod` exists.
...
When `footprint` is omitted, inference is intentionally strict:
