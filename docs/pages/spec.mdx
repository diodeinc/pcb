...
KiCad symbol/footprint repos are regular dependencies declared in `[dependencies]`, and workspace-level `[[workspace.kicad_library]]` entries define symbol/footprint/model linkage for resolution behavior.
...
For backwards compatibility, legacy `[assets]` entries that point into those repos are also used as version hints (repo root + version) when deriving KiCad dependency versions.
...
- Also accept KiCad's `<lib>:<fp>` form and resolve it using the matching `[[workspace.kicad_library]]` entry (`symbols` -> `footprints`) for the symbol's selected version.
- If no matching `[[workspace.kicad_library]]` selector exists, inference fails with an error.
