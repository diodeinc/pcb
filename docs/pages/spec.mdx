...
def check_resistor_values(nets, ports, components):
    ...
    for name, attrs in components.items():
        if "R" in name:  # Is a resistor
            if "Value" not in attrs and "Resistance" not in attrs:
                print(f"✗ {name} missing resistance value")
                return False
...
def check_differential_pairs(nets, ports, components):
    ...
    for pos, neg in diff_pairs:
        if pos not in nets or neg not in nets:
            print(f"✗ Differential pair {pos}/{neg} not found")
            return False
        # Check they connect to same component
        pos_comps = {p.split('.')[0] for p in nets[pos]}
        neg_comps = {p.split('.')[0] for p in nets[neg]}
        if pos_comps != neg_comps:
            print(f"✗ Differential pair {pos}/{neg} not routed to same components")
            return False
    return True
