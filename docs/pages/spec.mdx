...
- KiCad 10: `version = "10.0.3"` and `models.KICAD10_3DMODEL_DIR = "gitlab.com/kicad/libraries/kicad-packages3D"`

`@stdlib` separately pins its implicit managed KiCad asset dependencies to KiCad `10.0.3`.
...
- `pcb-version`: Compatible `pcbc` toolchain release lane (e.g., `"0.3"`) or the explicit moving `"nightly"` channel. For semver lanes, `pcb` selects the newest installed toolchain in that lane, installing the newest available patch release for the lane when none is installed. It never auto-upgrades a workspace across lanes such as `"0.3"` to `"0.4"`; run `pcb migrate` to make breaking-version upgrades explicit. Required for workspaces.
...
- `members`: Glob patterns matching subdirectories that contain local packages
...
`Net(name_or_net=None, voltage=None, impedance=None)` — all parameters optional. The first argument is positional-only and may be a name string or an existing net to cast. `Power` and `Ground` additionally accept a `voltage` parameter. Additional net types (`Analog`, `Pwm`, `Gpio`) are available from `@stdlib/interfaces.zen`.
...
Explicit constructor names still win, and unassigned constructor calls are rejected:
...
Regular net names must be unique. Duplicate regular net names are rejected; `NotConnected` nets are exempt.
`NotConnected` nets are source-unnamed; explicit names are ignored with a warning, and downstream tools assign connection-derived names where needed.
...
Net types follow automatic conversion rules across `io()` boundaries: `NotConnected` promotes to any type (universal donor), any specialized type demotes to `Net`, but `Net` cannot automatically promote to specialized types. For explicit casting, call the target constructor with an existing net as the first positional argument: `Power(net, voltage=Voltage("3.3V"))` or `Net(power_net)`.
