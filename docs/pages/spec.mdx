...
Remote package URLs omit versions. Declare each version in `pcb.toml`:
...
See [Packages](/pages/packages) for version resolution and `pcb sync`.
...
- `name`: Optional Diode workspace name override for board release uploads. When omitted, publish uses the first path segment of `repository`.
- `repository`: Git remote URL used to derive package URLs
- `pcb-version`: Required toolchain lane (`"0.3"`, `"0.4"`, or `"nightly"`)
- `endpoint`: Optional Diode host suffix for workspace-scoped app/API URLs
- `[board]`: The repository's buildable board (`name`, entry `path`, `description`)

Only the workspace root `pcb.toml` may contain a `[workspace]` section.
Board dependencies are declared in the root manifest's `[dependencies]` table.
...
Prefer generic components over raw `Component()` where possible. Generics come with standard symbols and footprints, and their normalized parameters are sent to the BOM API for matching.
...
Parent values convert to the declared type when possible. Strings coerce to primitives (`"true"` → `True`, `"42"` → `42`, `"3.3"` → `3.3`), physical quantities (`"10k"` → `Resistance("10k")`), and enum variants (`"0603"` → `Package("0603")`). When `allowed` is present, both the allowed set and the provided value use that same coercion path before membership is checked.
