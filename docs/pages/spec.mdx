...
Components generated by `pcb import` can carry incomplete sourcing provenance when the KiCad source does not provide enough information for `Part(...)`. `pcb build` reports these components with the `bom.imported_incomplete` warning instead of fabricating sourcing data or treating them as intentionally excluded from the BOM. Replace the generated component or add `Part(...)` to complete the design.
