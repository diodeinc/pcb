...
`implies` forms a DAG (an interface may imply several others), and matching is
nominal: two structurally identical interfaces remain distinct capabilities.
...
A `peripheral()` providing `Uart` (directly or through `implies`) may then
declare `attrs = {"baud_max": "8MHz"}` — and only that: an undeclared key
(`baudMax`, `max_baudrate`...) or a value of the wrong dimension (`"3.3V"`
for a `Frequency`) is an elaboration error naming the declared vocabulary.
Attribute names, like capability names, are declared once — in the
interface — not reinvented per component. The stdlib declares a consistent
rate vocabulary: `clk_max` on clocked buses (`Spi`, `I2c`, `Sdio`, `Jtag`,
memory buses...), `baud_max` on asynchronous serial (`Uart`, `Rs232`,
`Rs422`), `bitrate_max` on encoded serial links (`Can`, `Usb2`/`Usb3`,
`Pcie`, `Ethernet`, display/camera lanes...), plus `freq_max` on `OscPair`
and `gbw` on `Opamp`. Single-ended logic buses also declare `vio` (Voltage) —
for blocks whose logic level is **fixed** (level shifters, fixed-rail
transceivers, memory standards). A VDD-dependent block (an MCU UART runs at
whatever VDD is) must NOT declare `vio`: the per-design truth is the
**voltage on the nets**, and net/io voltage contracts — not capability
attrs — are what reject a 3.3V-to-5V connection between components.
...
fixed-function pinouts (AVR), alternate-function matrices (STM32), near-total
GPIO matrices (ESP32), and classic gate/pin swap, with one model.
...
GPIOS = pool("GPIO", provides = [Gpio], pins = ["PB4", "PB5", "PD6"])

result = pin_solve(
    [USART1, GPIOS],
    [
        pin_request("DEBUG", Uart),                       # any provider of Uart
        pin_request("LED", Gpio, prefer = ["PB4"]),
        pin_request("SC", Usart),                         # needs CK: USART-class only
    ],
)
```

- **`pin(name, data=, cost=, input_only=, strap=)`** — one candidate physical
  pin. `data` is opaque *realization* information — whatever downstream
  tooling needs to realize this candidate (an STM32 AF index `{"af": 1}`, an
  ESP32 IOMUX FUNC number `{"iomux_func": 0}`) — never consulted by the
  solver, splatted verbatim next to `"pin"` in the solved assignment.
  `cost` biases the solver (e.g. ESP32 IOMUX-direct = 0, matrix = 1);
  `strap` pins are penalized and warn when chosen; `input_only` pins are
  excluded from `direction = "output"` requests.
- **`peripheral(name, provides=, signals=, rebind=, attrs=, symmetric=, unless=)`**
  — a resource cluster. Every signal of every provided interface (closure
  included) must have a non-empty candidate list — a declaration cannot
  overstate the datasheet. `unless = "axis"` disables the peripheral when that
  key is truthy in `pin_solve(config=...)` (e.g. ESP32 ADC2 vs. WiFi).
  `rebind` records what a later swap costs: `"none"` (gate swap, no firmware),
  `"firmware"` (a constant to regenerate), `"fixed"` (never movable).
- **`pool(name, provides=, pins=, rebind=)`** — sugar for N interchangeable
  single-signal units (GPIO pools).
- **`pin_request(name, Iface, uses=, instance=, prefer=, lock=, where=, direction=, if_connected=, bind=)`**
  — a demand: the interface type *is* the request. Only `uses=` signals
  consume pins (default: all of the interface's signals). `where=` is a
  predicate over the peripheral's `attrs` (`lambda a: a["baud_max"] >= "1MHz"`
  works — unit strings coerce in physical-value comparisons). Attr keys and
  dimensions are reliable: they are validated against the vocabulary the
  provided interfaces declare (see `interface(attrs=...)` above). `instance=`,
  `prefer=`, `lock=` are caller-side overrides. With `if_connected = True`
  the request is served only when the caller actually connected the module
  input of the same name — pair it with `io(Iface, optional = True)` to
  pre-declare capability slots that consume resources only when used.
- **`pin_solve(peripherals, requests, config=, previous=)`** — deterministic
  joint instance × pin matching by branch-and-bound: the returned assignment
  minimizes total cost, not merely the first feasible one. Exclusivity is
  structural at both tiers: a pin carries at most one function, and an
  instance serves at most one request (two `Uart` requests can never share
  USART1, even on disjoint pin sets). `previous=` (a prior assignment) acts
  as a stability seed so adding a request does not reshuffle an already-routed
  design. Infeasibility is a build error with per-candidate rejection reasons.
  The cost of a candidate combines, in dominance order: surplus capability
  traits of the instance (+100 each — prefer the poorest sufficient
  instance), strapping pins (+50 each), per-pin `cost`, `prefer=` hits
  (−10 each), and previous-assignment matches (−20 instance, −5 per pin).
  Absolute values are arbitrary; only the relative order is contractual.
- **`at(value, pins, soft=)`** — constrain which physical pin(s) a
  connection may use, on the connection itself:
  `Mcu(gpio = {"LED": at(led_net, "PA8")})` or `Mcu(DEBUG = at(uart, ["PB6", "PB7"]))`.
  The wrapper is transparent — `io()` (or `pin_request(bind=)`) unwraps it,
  the inner value flows as if passed directly, and the constraint reaches
  `pin_solve`.
  Hard by default (unsatisfiable → build error, and the pin leaves the swap
  classes); `soft = True` is a preference the solver may fall back from.
- **`pin_map(assignment, ifaces)`** — turn `pin_solve`'s `assignment` into a
  `Component(pins=...)`-ready dict mapping each physical pin to the net
  carried by the matching interface field (`{"COM": uart_io, "LED": led_net}`;
  a bare `Net` is accepted for single-signal requests). Requests absent from
  the assignment — e.g. dropped by `if_connected` — are skipped, so both
  dicts can be declared unconditionally.

Two conventions keep the model sound. **Capabilities name hardware
peripherals** — `Gpio`, `AdcIn`, `Uart` are facts about the silicon;
application roles (LED, buzzer, button) are design-side labels and must never
become capability interfaces. And **pooled capabilities are demanded through
dicts, not numbered slots**: the component takes `gpio = config(dict,
default = {})` and fans each entry into a request
(`[pin_request(n, Gpio, bind = gpio[n]) for n in gpio]`), so the design names
its roles (`Mcu(gpio = {"LED": at(led_net, "PA8")})`) in whatever number it
needs. `bind=` carries the connected value on the request itself — an `at()`
wrapper contributes its pin constraint and the inner net flows to `pin_map`.
Named `io()` slots remain for singular ports (`SWD`, a debug `Uart`) where
the name is meaningful.

`pin_solve` returns `{"assignment": ..., "swap_classes": ..., "free_pins": [...]}`
— `free_pins` lists every candidate pin no served request claimed, so a
component wires its unused mux pins (`{p: NotConnected() for p in
res["free_pins"]}`) without maintaining a separate pin list. It stores the
first two
as the module properties `pin_assignment` and `swap_classes` (JSON), which
flow through the netlist into schematic fields: the chosen assignment
(including per-pin realization `data`, sufficient for firmware pin-init generation) and the
residual swap freedom (pool classes with spare pins, `rebind = "none"`
clusters for gate swap) reach layout tools with no side files.
