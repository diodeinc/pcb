...
parts = [
  { mpn = "TPS54331DR", symbol = "TPS54331.kicad_sym", manufacturer = "Texas Instruments", qualifications = ["Preferred"] },
]
```
...
- `parts`: Optional default sourcing metadata keyed by symbol file. Each entry provides `mpn`, `manufacturer`, optional `qualifications`, and a package-relative `.kicad_sym` `symbol` path. `Component()` sourcing precedence is described below.
