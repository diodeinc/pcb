...
**Example - Assign Manufacturer and MPN:**
```python
def assign_resistor_parts(component):
    """Assign specific part numbers for generic resistors"""
    if hasattr(component, "resistance"):
        ...
        if resistance == "10k":
            ...
            component.manufacturer = "Yageo"
            component.mpn = "RC0603FR-07100KL"

builtin.add_component_modifier(assign_resistor_parts)

# Child modules create generic resistors, parent assigns real parts
Child = Module("Child.zen")
Child(name="Board1")
```

**Example - Assign Capacitor Parts:**
```python
def assign_capacitor_parts(component):
    """Assign ceramic capacitors from approved vendor"""
    if hasattr(component, "capacitance"):
        cap = str(component.capacitance)
        voltage = str(component.voltage_rating) if hasattr(component, "voltage_rating") else "50V"

        if cap == "10uF" and voltage == "16V":
            component.manufacturer = "Samsung"
            component.mpn = "CL21A106KAYNNNE"

builtin.add_component_modifier(assign_capacitor_parts)
```
