...
def verify_power_connections(nets, ports, components):
    """Ensure all power pins are connected to VCC"""
    vcc_ports = [p for p, net in ports.items() if "VCC" in net]
    check(len(vcc_ports) >= 2, "Need at least 2 VCC connections")

def verify_ground_plane(nets, ports, components):
...
Check functions receive three dictionaries and must return a boolean:

```python
def check_function(
    nets: dict[str, list[str]],      # Map of net names to connected ports
    ports: dict[str, str],            # Map of port names to their nets
    components: dict[str, dict[str, str]]  # Map of component names to attributes
) -> bool:
    # Perform validation
...
- `nets`: Maps each net name to a list of connected port paths (e.g., `{"VCC": ["U1.VDD", "C1.P1"]}`)
- `ports`: Maps each port path to its connected net (e.g., `{"U1.VDD": "VCC"}`)
- `components`: Maps component paths to their attributes including pins, values, and properties
...
**Example with Detailed Checks:**

```python
def check_resistor_values(nets, ports, components):
    """Verify all resistors have valid values"""
    for name, attrs in components.items():
        if "R" in name:  # Is a resistor
            check("Value" in attrs or "Resistance" in attrs, f"{name} missing resistance value")
            print(f"✓ {name} has resistance: {attrs.get('Value', attrs.get('Resistance'))}")

def check_differential_pairs(nets, ports, components):
    """Verify differential pairs are properly routed"""
    diff_pairs = [("USB_DP", "USB_DN"), ("TX_P", "TX_N")]
    for pos, neg in diff_pairs:
        check(pos in nets and neg in nets, f"Differential pair {pos}/{neg} not found")
        # Check they connect to same component
        pos_comps = {p.split('.')[0] for p in nets[pos]}
        neg_comps = {p.split('.')[0] for p in nets[neg]}
        check(pos_comps == neg_comps, f"Differential pair {pos}/{neg} not routed to same components")
...
TestBench(
    ...
    checks = [
        ...
        # Can mix named functions and lambdas
        lambda n, p, c: "GND" in n,  # Ensure ground exists
    ]
)
```
