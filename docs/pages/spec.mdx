...
- `parts`: Optional default sourcing metadata keyed by symbol file. Each entry provides `mpn`, `manufacturer`, and a package-relative `.kicad_sym` `symbol` path. `Component()` sourcing precedence is described below.
