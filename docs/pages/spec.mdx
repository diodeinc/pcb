...
Part resolution precedence for canonical sourcing fields:

1. `part=` (`part.mpn` / `part.manufacturer`) when provided
2. explicit `mpn=` / `manufacturer=`
3. legacy `properties` values (`mpn`/`Mpn`/`manufacturer`)
...
- when `part` is provided, it overrides scalar `mpn`/`manufacturer` values.
