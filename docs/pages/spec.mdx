...
Each matcher function receives `(path, cursor_index)` and returns the number of components consumed:

```python
def custom_matcher(path, cursor):
    # Validate component at current cursor position
