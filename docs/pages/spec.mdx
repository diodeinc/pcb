...
- **`pin(name, af=, cost=, input_only=, strap=)`** — one candidate physical
  pin. `cost` biases the solver (e.g. ESP32 IOMUX-direct = 0, matrix = 1);
...
(including `af=` indices, sufficient for firmware pin-init generation) and the
