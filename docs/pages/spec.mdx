...
GpioPin = interface(PIN = Net)   # stdlib `Gpio` is a net type, not a capability
...
| `strap` | no | Boot-strapping pin: chosen only as a last resort, warns when used |
...
| `unless` | no | The peripheral is unavailable when this key is truthy in `pin_solve(config=...)` |
...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `name` | yes | Request name; keys the result and the `if_connected` pairing |
...
| `where` | no | Predicate over the provider's attrs: `lambda a: a["baud_max"] >= "1MHz"` |
| `instance` | no | Restrict to a named peripheral or pool |
| `prefer` | no | Pins the solver favors when choosing |
| `lock` | no | Make `prefer` mandatory instead of preferred |
| `direction` | no | `"input"` or `"output"`; `"output"` excludes `input_only` pins |
| `if_connected` | no | Serve the request only when the caller connected the module input of the same name; pair with `io(Iface, optional = True)` |
...
joint instance × pin assignment. A pin carries at most one function and an
...
honors per-pin `cost` and `prefer=`, and sticks to `previous=` (a prior
assignment passed as a stability seed) so adding a request does not reshuffle
a routed design.
...
reaches `pin_solve`. Hard by default (unsatisfiable → build error);
`soft = True` is a preference the solver may fall back from.

**`pin_map(assignment, ifaces)`** — turn `pin_solve`'s `assignment` into a
`Component(pins=...)`-ready dict (physical pin → net of the matching
interface field; a bare `Net` works for single-signal requests). Requests
absent from the assignment (e.g. dropped by `if_connected`) are skipped.

Two conventions keep the model sound. **Capabilities name hardware
peripherals** — `GpioPin`, `AdcIn`, `Uart` are facts about the silicon;
application roles (LED, buzzer) are design-side labels, never capability
interfaces. And **pooled capabilities are demanded through dicts, not
numbered slots**: the component takes `gpio = config(dict, default = {})`
and fans each entry into `pin_request(n, GpioPin, bind = gpio[n])`, so the
design names its roles (`Mcu(gpio = {"LED": at(led_net, "PA8")})`) in
whatever number it needs. Named `io()` slots remain for singular ports
(`SWD`, a debug `Uart`).
...
| Key | Content |
|-----|---------|
| `assignment` | Per request: the chosen instance and pin per signal (with each pin's realization `data`), plus free alternates |
| `swap_classes` | Residual swap freedom: pool classes with spare pins, gate-swap clusters |
| `free_pins` | Candidate pins no request claimed — wire them with `{p: NotConnected() for p in res["free_pins"]}` |

`assignment` and `swap_classes` are also stored as module properties (JSON)
and flow through the netlist into schematic fields, so the assignment and the
