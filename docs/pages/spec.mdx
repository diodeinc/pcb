...
parts = [
  { mpn = "TPS54331DR", symbol = "TPS54331.kicad_sym", manufacturer = "Texas Instruments" },
]
```
...
- `parts`: Optional default sourcing metadata keyed by symbol file. Each entry provides `mpn`, `manufacturer`, and a package-relative `.kicad_sym` `symbol` path. When a `Component()` uses that symbol and does not set `part`/`mpn`, the first entry becomes the primary part and later entries become `alternatives`.
...
When `footprint` is omitted, it is inferred from the symbol's metadata. When `part` is provided, its `mpn` and `manufacturer` override any scalar `mpn`/`manufacturer` parameters or symbol metadata. If neither `part` nor scalar sourcing is set, `Component()` can inherit a default part from the package manifest's `parts` entries matching the symbol path.
