...
2. **Build-time extraction** — CLI parses changelog at compile time for `pcb doc --changelog`, including focused `--latest` and `--unreleased` views. Expects `## [...]` version headers and `### Category` subsections.
