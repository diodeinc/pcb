...
Wrap reference designators with a `REF:` prefix inside backticks: `` `REF:R1` ``, `` `REF:C5` ``, `` `REF:U3` ``. The parser matches the `` `REF:...` `` pattern and links them to the PCB viewer so users can click to highlight the component on the board.

The `REF:` prefix avoids false positives from other backtick content like file names, shell commands, or version numbers.
...
The 47k ESTOP debounce resistor `REF:R_ESTOP_DEBOUNCE` creates a voltage
divider with the ESTOP LED, pulling the signal to ~1.5V (below the AND gate
VIH threshold of 2.0V). Short across `REF:R_ESTOP_DEBOUNCE` (47k 0402) with
...
`REF:C5` and `REF:C6` have insufficient 10uF decoupling for `REF:U1` LDO.
Replace both with 22uF/16V X5R (GRM188R61C226ME15). Bottom side, adjacent
to `REF:U1`.

### 3. CAN termination resistor

`REF:R12` footprint is unpopulated. Add 120 ohm 0402 resistor for CAN bus
termination.
...
|------|-----------|----------|-----------|
| 3.3V | `REF:TP1` | 3.30V | ±50mV |
| 1.8V | `REF:TP2` | 1.80V | ±30mV |
| 5.0V | `REF:TP3` | 5.00V | ±100mV |

### CAN Bus Test

1. Connect CAN analyzer to `REF:J2`
...
The parser also handles older formats found in existing files, including the structured-field format (`**Designators:**`, `**Status:**`, `**Problem:**`, etc.) and numbered lists without a `## Reworks` section. These are supported for backwards compatibility only — always use the format described above for new documentation.
