...
```toml
# Library X (not yet migrated)
[dependencies]
"github.com/diodeinc/stdlib" = "0.3"

# Library Y (migrated to new API)
[dependencies]
...
github.com/diodeinc/registry/components/ti/tps54331 v1.0.0 h1:ghi789jkl012...
github.com/diodeinc/registry/components/ti/tps54331 v1.0.0/pcb.toml h1:mno345pqr678...
```
...
2. Converts `pcb.toml` files to V2 format (adds root `[workspace].pcb-version`, removes package-level `[workspace]` sections, converts `[packages]` to `[dependencies]`)
...
Builds a board or workspace package. Automatically resolves dependencies,
...
pcb update path/to/package   # Update only that workspace package
```
...
If the `path` argument points at a discovered workspace package directory (a
directory containing a `pcb.toml` that is not excluded), `pcb update`
