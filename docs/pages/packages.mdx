...
Content hashes respect `.gitignore` and `.ignore` within the package root and
traversed subdirectories, in both repositories and archive extracts. Ancestor
rules, global Git exclusions, and `.git/info/exclude` do not affect file selection.
Previously published versions whose hashes depend on those rules may need a new
package version; existing tags are not rewritten.
