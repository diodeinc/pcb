...
`stdlib@0.3.x` and `stdlib@1.0.x`, each used by the packages that require them.
...
The `@1` and `@9` suffixes are compatibility lanes. They allow multiple
...
The toolchain-managed `@stdlib` package separately pins its implicit KiCad asset
dependencies to KiCad `10.0.0`.
...
assets are written to `[dependencies.indirect]` using lane-qualified keys:

```toml
[dependencies.indirect]
"gitlab.com/kicad/libraries/kicad-footprints@10" = "10.0.0"
"gitlab.com/kicad/libraries/kicad-packages3D@10" = "10.0.0"
"gitlab.com/kicad/libraries/kicad-symbols@10" = "10.0.0"
```
...
Upgrades an older project to the supported manifest format. Updates `pcb.toml`
manifests and rewrites `.zen` files to use the current import syntax. Older
workspace formats are no longer supported by `pcb build`, `pcb layout`,
`pcb publish`, etc.
...
The command detects the workspace root, converts manifests and import paths, and
hydrates dependency manifests.
...
removing imports, changing dependency versions, or migrating an older project.
...
`--locked` checks that the hydrated state is sufficient and does not rewrite
`pcb.toml`.
