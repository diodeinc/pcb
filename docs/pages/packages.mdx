...
A top-level package directory named `workspace` collides with the root
package's reserved URL when `[workspace].repository` is unset, so `pcb`
surfaces a discovery error instead of silently dropping the descendant.
Set `[workspace].repository` (or rename the directory) to disambiguate.
