...
Normalized BOM matches are cached independently per BOM line. `pcb bom` treats
entries updated within the last ten minutes as fresh, refreshes only stale or
missing lines, and keeps available stale matches if the refresh fails.
`pcb bom --offline` never contacts the API and uses each available cached line
