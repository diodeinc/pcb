...
Content hashes respect `.gitignore` in both the publisher's repository and the
consumer's archive extract, but ignore global Git exclusions and `.git/info/exclude`.
