...
Creates a local editable copy of a remote dependency for development. This is
useful when you need to modify a dependency without publishing changes upstream.

```bash
pcb fork github.com/diodeinc/registry/modules/UsbPdController
pcb fork github.com/diodeinc/registry/modules/UsbPdController --version 1.2.3
pcb fork github.com/diodeinc/registry/modules/UsbPdController --force
```
...
- Re-running `pcb fork` without `--force` is idempotent (preserves your changes)
...
```bash
pcb unfork github.com/diodeinc/registry/modules/UsbPdController
pcb unfork github.com/diodeinc/registry/modules/UsbPdController --keep-files
```

The command:
1. Removes the `[patch]` entry from `pcb.toml`
2. Deletes the fork directory (unless `--keep-files` is specified)
3. The next `pcb build` will resolve the remote version

**Flags:**
- `--keep-files`: Remove the patch but preserve the fork directory
