...
`[[workspace.kicad_library]]` entries.
The built-in KiCad library includes versions `9.0.3` and `10.0.0`.
...
If `[[workspace.kicad_library]]` is omitted, the toolchain seeds a built-in KiCad 9 entry with the configuration shown above.
...
`pcb build`, `pcb layout`, `pcb publish`, etc.
...
pcb fork remove --all
pcb fork remove github.com/diodeinc/registry/modules/UsbPdController --force
```

The command:
1. Removes the `[patch]` entry from `pcb.toml`
2. Deletes the fork directory (if it's under `fork/` and has no uncommitted changes)
3. Cleans up empty parent directories
4. The next `pcb build` will resolve the remote version

**Flags:**
- `--all`: Remove all forked packages (all path patches)
- `--force`: Delete fork directories even if they have uncommitted changes
...
```bash
pcb fork upstream
pcb fork upstream --dry-run
```

The command:
1. Collects all path patches from `pcb.toml`
2. Copies forked files to the corresponding locations in the registry
3. Pushes changes to a new branch
4. Provides a PR creation URL
