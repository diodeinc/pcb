...
Registry-backed `pcb search` defaults to the public Diode registry and registries in the same Git workspace as the board repository.

For example, a board repository at `code.diode.computer/weave/WW0001` searches `github.com/diodeinc/registry` plus accessible registries whose URLs start with `code.diode.computer/weave/`.
