# Building an Antenna CRC Matcher with Zener

If we pull it off with Zener, it should end up looking like the three KiCad schematics below!
...
```
> [!WARNING]
> In the future, variables may not be able to use symbols, so you should inline define symbols when defining components like later in this tutorial.
...
After rotating and moving around the rendered schematic exactly as you would with KiCad:
