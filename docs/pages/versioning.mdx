...
The limitation is that types from different major versions are incompatible. A
`Net` from stdlib 0.3 cannot be passed to a function expecting a stdlib 1.0 `Net`.
This is intentional—major versions represent breaking changes, and the system
