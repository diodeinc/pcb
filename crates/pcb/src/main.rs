...
fn install_shim_launcher(latest: &LatestRelease) -> Result<()> {
    ...
    let client = http_client(ARCHIVE_TIMEOUT)?;
    let mut download = None;
    let mut launcher_current = false;
    for target in download_target_triples().iter().copied() {
        ...
        // Skip re-downloading when the on-disk launcher already matches the
        // release, but still fall through to `--install` so protocol-handler
        // registration (and the macOS app bundle copy) can be repaired.
