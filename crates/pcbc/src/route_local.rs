//! PCB local auto-routing command via FreeRouting
//!
//! Routes boards locally using a FreeRouting Java JAR.
//!
...
pub fn execute(args: RouteLocalArgs) -> Result<()> {
    ...
    route_local(&args, &board_path, &board_name)
}

/// Evaluate .zen, find .kicad_pcb, validate.
...
fn route_local(args: &RouteLocalArgs, board_path: &Path, board_name: &str) -> Result<()> {
    ...
    // 2. Session files live alongside the board; cleaned up on success/failure
    ...
    let mut clear_existing = args.force;
    if (traces > 0 || vias > 0) && !args.force {
        let route_desc = match (traces, vias) {
            (t, 0) => format!("{} trace{}", t, if t == 1 { "" } else { "s" }),
            (0, v) => format!("{} via{}", v, if v == 1 { "" } else { "s" }),
            (t, v) => format!(
                "{} trace{} and {} via{}",
                t,
                if t == 1 { "" } else { "s" },
        ...
        let confirmed = inquire::Confirm::new("  Clear existing routes and re-route?")
            .with_default(false)
            .prompt()
            .unwrap_or(false);
        if !confirmed {
    ...
    // 8. Remove backup and session files on success
    if did_clear {
        let _ = std::fs::remove_file(&backup_path);
    }
    let _ = std::fs::remove_file(&dsn_path);
...
/// Delete all tracks and vias from the board via `pcbnew`.
...
/// Import a Specctra SES session file into the board via `pcbnew`.
...
/// Run the full DSN → FreeRouting → SES import chain.
fn run_local_route_chain(
    ...
    import_ses(board_path, ses_path)?;
    spinner.finish();
...
fn download_with_retry(url: &str, max_attempts: u32) -> Result<Vec<u8>> {
    ...
    for attempt in 1..=max_attempts {
        ...
        let mut reader = response.take(200_000_000);
