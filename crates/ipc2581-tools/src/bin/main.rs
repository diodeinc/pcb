...
use ipc2581::{Ipc2581, LayerFunction, PlatingStatus};
...
fn check_file(path: &PathBuf, verbose: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let is_reduced_layer_file = matches!(
        ...
        ipc2581::Mode::Assembly | ipc2581::Mode::Test | ipc2581::Mode::Stencil
    );
    ...
    if let Some(bom) = doc.bom() {
        ...
        for item in &bom.items {
            match item.category {
                Some(ipc2581::BomCategory::Mechanical) => {
                    mechanical_qty += item.quantity.unwrap_or(0);
                    mechanical_types += 1;
                }
                Some(ipc2581::BomCategory::Electrical) => {
...
fn export_svg(
    ...
    if let Some(debug_path) = debug_stage3_path {
        ...
        for (layer_name, paths) in &layer_paths {
            ...
            use ipc2581_tools::svg_export::debug::export_layer_paths_svg;
    ...
    if let Some(bucket_name) = debug_bucket {
        use ipc2581_tools::svg_export::debug::export_bucket_svg;
    ...
    if let Some(debug_path) = debug_stage4_path {
        ...
        for (layer_name, flattened) in &flattened_layers {
            ...
            use ipc2581_tools::svg_export::debug::export_flattened_svg_with_drill_mask;
