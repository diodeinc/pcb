...
class VirtualFootprint(VirtualElement):
    ...
    def replace_with(self, source_footprint: "VirtualFootprint") -> None:
        ...
        if source_footprint.kicad_footprint and self.kicad_footprint:
            ...
            # Invalidate parent's cached bounding box since our position changed
            self._invalidate_parent_bbox()

    def _invalidate_parent_bbox(self) -> None:
        """Invalidate parent group's cached bounding box recursively."""
        if self.parent and isinstance(self.parent, VirtualGroup):
            self.parent._cached_bbox = None
            self.parent._invalidate_parent_bbox()
...
class ImportNetlist(Step):
    ...
    - Mapping lens tracking to SyncState tracking
    - Building the virtual DOM after sync
    """
    ...
    def run(self):
        ...
        if result.applied:
            ...
            for uuid_str in tracking.get("removed_uuids", set()):
                self.state.removed_footprint_uuids.add(uuid_str)
