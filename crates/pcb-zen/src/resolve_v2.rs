...
pub fn ensure_sparse_checkout(
    ...
    let marker = if add_v_prefix { "pcb.toml" } else { ".pcb-cached" };
    ...
    populate_cache(checkout_dir, marker, |staging| {
        ...
        if !is_pseudo_version && subpath.is_empty() {
            ...
            if let Some((url_pattern, root_pattern)) = crate::archive::get_archive_pattern(host) {
                if crate::archive::fetch_archive(url_pattern, root_pattern, repo_url, &ref_spec, staging).is_ok() {
...
fn fetch_via_git(staging: &Path, repo_url: &str, ref_spec: &str, subpath: &str, is_pseudo: bool) -> Result<()> {
    ...
    git::run_in(staging, &["config", "remote.origin.partialclonefilter", "blob:none"])?;

    let fetch_ref = if is_pseudo { ref_spec.to_string() } else { format!("refs/tags/{}", ref_spec) };
    let fetch_args = ["fetch", "--depth=1", "--filter=blob:none", "origin", &fetch_ref];
    if git::run_in(staging, &fetch_args).is_err() {
        git::run_in(staging, &["remote", "set-url", "origin", &git::format_ssh_url(repo_url)])?;
