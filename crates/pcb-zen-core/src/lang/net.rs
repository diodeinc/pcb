...
use starlark::{
    ...
    values::{
        ...
        Coerce, Freeze, FreezeResult, FrozenValue, StarlarkValue, Trace, Value, ValueLike,
    },
};
...
impl<'v, V: ValueLike<'v>> std::fmt::Display for NetValueGen<V> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // Match old API behavior - delegate to Debug for backwards compatibility
        write!(f, "{self:?}")
    }
}
...
impl<'v, V: ValueLike<'v>> DeepCopyToHeap for NetValueGen<V> {
    fn deep_copy_to<'dst>(&self, dst: &'dst starlark::values::Heap) -> anyhow::Result<Value<'dst>> {
        let properties = self
            .properties
            .iter()
            .map(|(k, v)| {
