...
mod tests {
    ...
    // End-to-end: when `[workspace].repository` is unset and a descendant sits at
    // top-level rel-path `workspace`, `build_resolution_map` must register the
    // descendant's package directory so its dependencies are resolved rather than
    // silently dropped. Before the fix, the root-sentinel URL ("workspace")
    // collision in `get_workspace_info` overwrote the discovered descendant in
    // `workspace.packages`, so `build_resolution_map` (which iterates
    // `workspace.packages.values()`) never inserted the descendant's dir.
    #[test]
    fn build_resolution_map_includes_colliding_descendant_package_dir() {
        use crate::workspace::get_workspace_info;

        struct NullResolver {
            closure: HashMap<ModuleLine, Version>,
        }
        impl PackagePathResolver for NullResolver {
            fn resolve_package(&self, _module_path: &str, _version: &str) -> Option<PathBuf> {
                None
            }
            fn selected_versions(&self) -> &HashMap<ModuleLine, Version> {
                &self.closure
...
"#
        ...
        let workspace = get_workspace_info(&provider, Path::new("/repo")).unwrap();

        // The fix surfaces a collision error instead of silently overwriting.
        assert_eq!(workspace.errors.len(), 1);
        assert!(workspace.errors[0].error.contains("collides"));

        let resolver = NullResolver {
            closure: HashMap::new(),
        };
        let results = build_resolution_map(&provider, &resolver, &workspace, &HashMap::new());

        // The descendant's package dir MUST be a key in the resolution map; before
        // the fix it was absent because the descendant was overwritten in
        // `workspace.packages` and `build_resolution_map` only iterates
        // `workspace.packages.values()`.
        let descendant_dir = Path::new("/repo").join("workspace");
        assert!(
            results.contains_key(&descendant_dir),
            "descendant package dir {descendant_dir:?} must be in the resolution map; got keys: {:?}",
            results.keys().collect::<Vec<_>>()
        );
        // The root is still resolved via the workspace-root fallback branch in
        // `build_resolution_map`, because the colliding root insert was skipped
        // (so the root is not in `packages`, `has_root_package` is false, and the
        // `results.entry(workspace.root)` fallback fires).
        assert!(
            results.contains_key(Path::new("/repo")),
            "workspace root must still be in the resolution map; got keys: {:?}",
            results.keys().collect::<Vec<_>>()
        );
    }
}
