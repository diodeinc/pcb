...
use crate::{
    geometry, FeatureSet, LayerFeature, LayerFunction, LineDesc, PadStackDef, PadUse, Polarity,
    PolyStep, Polygon, StandardPrimitive, Symbol,
};
use geo::{BooleanOps, Coord, LineString, MultiPolygon, Polygon as GeoPolygon};
use lyon_path::{math::Point as LyonPoint, Path as LyonPath};
use lyon_tessellation::{
    BuffersBuilder, LineCap, LineJoin, StrokeOptions, StrokeTessellator, StrokeVertex,
    VertexBuffers,
};
use std::collections::HashMap;
use svg::node::element::{path::Data, Circle, Definitions, Group, Mask, Path, Rectangle};
use svg::Document;
...
pub fn render_copper_layer_svg(
    ...
    let pos_data =
        geo_polygons_to_svg_data(&positive_geometry, min_x, min_y, scale, offset_x, offset_y);
    ...
    if !negative_geometry.is_empty() {
        let neg_data =
            geo_polygons_to_svg_data(&negative_geometry, min_x, min_y, scale, offset_x, offset_y);
    ...
    for (slot_outline, x_offset_slot, y_offset_slot) in board_geom.slots {
        let slot_path_data = add_polygon(
            Data::new(),
            slot_outline,
            *x_offset_slot,
            *y_offset_slot,
    ...
    for (x, y, hole_diameter, pad_shape) in board_geom.pths {
        ...
        if matches!(layer_side, LayerSide::Top | LayerSide::Bottom) {
            if let Some(pad_poly) = padshape_to_polygon(pad_shape, *x, *y) {
                let pad_data =
                    geo_polygons_to_svg_data(&[pad_poly], min_x, min_y, scale, offset_x, offset_y);
...
fn extract_set_geometry(
    ...
    for pad in &set.pads {
        if let (Some(x), Some(y)) = (pad.x, pad.y) {
            let rotation = pad.xform.map(|xf| xf.rotation).unwrap_or(0.0);
...
fn stroke_trace_to_polygons(trace: &crate::Trace, line_width: f64) -> Option<Vec<GeoPolygon<f64>>> {
    ...
    for i in 0..trace.points.len() - 1 {
        ...
        let corners = vec![
            Coord {
                x: p1.x + px * half_width,
                y: p1.y + py * half_width,
            },
            Coord {
                x: p2.x + px * half_width,
                y: p2.y + py * half_width,
            },
            Coord {
                x: p2.x - px * half_width,
                y: p2.y - py * half_width,
...
fn ipc_polygon_to_geo_polygon(
    ...
    let mut coords = Vec::new();
    coords.push(Coord {
        x: polygon.begin.x + offset_x,
        y: polygon.begin.y + offset_y,
    });
...
fn primitive_to_polygon(
    ...
    match primitive {
        ...
        StandardPrimitive::RectCenter(rect) => Some(create_rectangle_polygon(
            x,
            y,
            rect.width,
            rect.height,
        ...
        StandardPrimitive::RectRound(rect) => Some(create_rounded_rect_polygon(
            x,
            y,
            rect.width,
            rect.height,
...
fn padshape_to_polygon(
    shape: &crate::board_outline::PadShape,
    x: f64,
    y: f64,
) -> Option<GeoPolygon<f64>> {
    ...
    match shape {
        PadShape::Circle { diameter } => Some(create_circle_polygon(x, y, diameter / 2.0, 32)),
        PadShape::Rect { width, height } => {
            Some(create_rectangle_polygon(x, y, *width, *height, 0.0))
        }
        PadShape::Oval { width, height } => Some(create_oval_polygon(x, y, *width, *height, 0.0)),
        PadShape::Polygon { polygon } => ipc_polygon_to_geo_polygon(polygon, x, y),
...
fn create_rounded_rect_polygon(
    cx: f64,
    cy: f64,
    width: f64,
    height: f64,
