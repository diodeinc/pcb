...
impl<'arena> Ipc2581<'arena> {
    ...
    pub fn parse(arena: &'arena Bump, xml: &str) -> Result<Self> {
    ...
    pub fn parse_file(arena: &'arena Bump, path: impl AsRef<Path>) -> Result<Self> {
        let xml = std::fs::read_to_string(path)?;
        Self::parse(arena, &xml)
    }
...
<IPC-2581 revision="C" xmlns="http://webstds.ipc.org/2581">
  <Content roleRef="Owner">
    ...
    <DictionaryUser units="MILLIMETER"/>
        ...
        let arena = Bump::new();
        let result = Ipc2581::parse(&arena, xml);
