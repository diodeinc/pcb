...
fn get_bom_stats() -> (u32, u32, usize, usize) {
    ...
    let arena = Bump::new();
    let Ok(bom_doc) = Ipc2581::parse_file(&arena, bom_path) else {
        return (0, 0, 0, 0);
    };
        ...
        Ok(doc) => {
            ...
            assert!(
                matches!(
                    ...
                    ipc_2581::Mode::UserDef
                        | ipc_2581::Mode::Bom
                        | ipc_2581::Mode::Stackup
                        | ipc_2581::Mode::Fabrication
                        | ipc_2581::Mode::Assembly
                        | ipc_2581::Mode::Test
                        | ipc_2581::Mode::Stencil
                        | ipc_2581::Mode::Dfx
                ),
...
fn test_testcase1_metadata() {
    use ipc_2581::{LayerFunction, PlatingStatus};
...
fn test_testcase1_cross_file_consistency() {
    ...
    let full = Ipc2581::parse_file(
        &arena,
        Path::new("tests/data/Testcase1-RevC/testcase1-RevC-full.xml"),
    )
    .unwrap();
    let assembly = Ipc2581::parse_file(
        &arena,
        Path::new("tests/data/Testcase1-RevC/testcase1-RevC-Assembly.xml"),
    )
    .unwrap();
    let bom_doc = Ipc2581::parse_file(
        &arena,
        Path::new("tests/data/Testcase1-RevC/testcase1-RevC-BOM.xml"),
    )
    .unwrap();
