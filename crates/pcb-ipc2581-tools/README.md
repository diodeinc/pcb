...
panel stackup and canonical physical layer definitions. Each input must have one
simple clockwise rounded-rectangle root profile, as generated by
`board-array create`.

The generated panel applies the fabrication projection by default. Gerber
manufacturing export writes one `Fab_Panel_Profile.gm1` file that contains the
fab panel outline, nested board-array reliefs, and a routed material-removal
band around each placed assembly panel. The band accounts for a 1.0 mm cutter
outside the input profile, so the finished assembly panel keeps its requested
dimensions. These profile routes do not appear in `NPTH.drl`.
